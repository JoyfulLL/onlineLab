...
const handleRemoveClick = async () => {
  const confirmResult = await ElMessageBox.confirm('确定要移出所选学生的班级？', '提示', { type: 'warning' })
...
const removeFromClass = async row => {
  const { id } = row
  const confirmResult = await ElMessageBox.confirm('确定要移出所选学生吗？', '提示', {
    type: 'warning',
  })
  if (confirmResult === 'confirm') {
    // 用户点击了确认按钮,执行移出班级的操作
    ...
    if (query) {
      filtered = filtered.filter(item => {
        return (
          ...
          (item.class && typeof item.class === 'string' && item.class.toLowerCase().includes(query))
        )
      })
    }
...
<template>
  <div class="user-header">
    <el-form :inline="true">
      <el-form-item>
        <!--      移出班级的 按钮-->
        <el-button type="danger" size="default" @click="handleRemoveClick" :disabled="!isAnyStudentSelected">
          批量移出班级
        </el-button>
      </el-form-item>
    </el-form>
    ...
    <!--    用于老师查看学生的信息 教师无权限编辑-->
    <el-dialog v-model="dialogVisibleViewStu" title="查看学生" width="30%" :before-close="handleClose">
      <!-- 查看学生信息用-->
      <div class="form-container">
        <student-registration :showPassword="false" :IsDisabled="true" :userData="userForm" />
      </div>
    </el-dialog>

    <!--    用于管理员编辑学生信息-->
    <el-dialog v-model="dialogVisibleEditStudent" title="编辑学生" width="30%" :before-close="handleClose">
      <div class="form-container">
        <student-registration :showPassword="false" :IsDisabled="IsDisabled.value" :userData="userForm"
          :action="MyEditAction" @edit-success="closeEditDialog" />
      </div>
    </el-dialog>
    ...
    <el-form :inline="true">
      <el-form-item>
        <el-input class="w-50 m-2" placeholder="搜索学生姓名/班级" v-model="queryInfo" clearable>
          <template #prefix>
  ...
  <div class="table">
    <el-table :data="filteredData" style="width: 100%" border stripe max-height="600"
      ...
      <el-table-column prop="class" label="班级" width="150" :filters="classFilters" :filter-method="filterClasses"
      ...
      <el-table-column fixed="right" label="操作" min-width="100">
        <template #default="scope">
          ...
          <el-button type="danger" size="default" @click="removeFromClass(scope.row)">移出班级</el-button>
        </template>
      </el-table-column>
    </el-table>
  </div>
  <!-- 分页 -->
  <el-pagination :page-size="pageSize" :page-sizes="[10, 20, 30, 50]" background default
    layout="total,prev, pager, next, sizes" :total="studentDataTable.studentsDataCount" @current-change="changePage"
